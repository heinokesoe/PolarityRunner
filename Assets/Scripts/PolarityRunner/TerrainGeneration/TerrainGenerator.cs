...
namespace PolarityRunner.TerrainGeneration
{
	...
	public abstract class TerrainGenerator : MonoBehaviour
	{
		...
		protected float m_RuntimeLevelLength = -1f;
		...
		{
			get { return m_RuntimeLevelLength > 0f ? m_RuntimeLevelLength : m_Settings.LevelLength; }
		}
		...
		{
			if ( m_CurrentX < LevelLength || LevelLength <= 0 )
			{
		...
		public virtual void Remove ()
		{
			...
			{
				float blockEndX = block.Value.transform.position.x + block.Value.Width;
				if ( IsOutsideCleanupRange ( m_Character.transform.position.x, blockEndX, m_DestroyRange ) )
				{
		...
		{
			return GetBlockAt ( m_Character.transform.position.x );
		}

		public Block GetBlockAt ( float worldX )
		{
			foreach ( KeyValuePair<Vector3, Block> block in m_Blocks )
			{
				if ( block.Key.x <= worldX && block.Key.x + block.Value.Width > worldX )
				{
					return block.Value;
				}
			}
		...
		public void ConfigureLevelLength ( float levelLength )
		{
			m_RuntimeLevelLength = Mathf.Max ( 1f, levelLength );
		}

		public static bool IsOutsideCleanupRange ( float playerX, float blockEndX, float destroyRange )
		{
			return playerX - blockEndX > Mathf.Max ( 0f, destroyRange );
		}
